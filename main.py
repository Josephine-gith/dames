    ...
    if plateau[coord_f] is None and ((coord_f[0] + coord_f[1]) % 2) == 1:
        if couleur == WHITE and tour % 2 == 0:
            if (coord_f[1] - coord_i[1]) == -1 and abs(coord_f[0] - coord_i[0]) == 1:
                ...
                abs(coord_f[1] - coord_i[1]) == 2 and abs(coord_f[0] - coord_i[0]) == 2
            ):
                if (
                    plateau[
                        (coord_f[0] + coord_i[0]) / 2, (coord_f[1] + coord_i[1]) / 2
                    ]
                    == BLACK
                ):
                    plateau[
...
def end_game(plateau, running):
    w, b = 0, 0
    for k in plateau.keys():
        if Cases[k] == BLACK:
            b = b + 1
        elif Cases[k] == WHITE:
            w = w + 1
...
if __name__ == "__main__":
    ...
    while running:
        ...
        running = end_game(Cases,running)
